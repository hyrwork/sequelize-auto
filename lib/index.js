...
AutoSequelize.prototype.run = function(callback) {
  ...
  function generateText(err) {
    ...
    async.each(_.keys(self.tables), function(table, _callback){
      ...
      var hasStateColumn = ((table !== 'state') && (fields.indexOf('state_id') > -1));
      ...
      text[table] += "const mixins = require('./_mixins');\n\n";
      text[table] += "module.exports = function(sequelize, DataTypes) {\n\n";
      text[table] += spaces + "let Model = sequelize.define('" + table + "', {\n\n";
      ...
      // text[table] += spaces + spaces  + "validate: {\n\n";
      // text[table] += spaces + spaces + spaces + "isValid: function(){if(false){throw new Error('Impossible!');}}";
      // text[table] += spaces + spaces  + "},\n\n";
      ...
      if(hasStateColumn){
        text[table] += spaces + spaces + spaces + spaces + "mixins.associate.belongsTo(Model, models.state);\n\n";
      }

      text[table] += spaces + spaces + spaces + "}" + (hasStateColumn ? ',' : '') + "\n\n";

      if(hasStateColumn){
        text[table] += spaces + spaces + spaces + "setStateByName: mixins.classMethods.setStateByName\n\n";
      }
      text[table] += spaces + spaces  + "},\n\n";

      text[table] += spaces + spaces  + "instanceMethods: {\n\n";
      if(hasStateColumn){
        text[table] += spaces + spaces + spaces + "setStateByName: mixins.instanceMethods.setStateByName\n\n";
      }
      text[table] += spaces + spaces  + "},\n\n";
