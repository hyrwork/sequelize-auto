...
AutoSequelize.prototype.run = function(callback) {
  ...
  function generateText(err) {
    ...
    async.each(_.keys(self.tables), function(table, _callback){
      ...
      if(hasStateColumn){
        text[table] += spaces + spaces + spaces + "setStateByKey: mixins.classMethods.setStateByKey\n\n";
      }
      text[table] += spaces + spaces  + "},\n\n";

      text[table] += spaces + spaces  + "instanceMethods: {\n\n";
      if(hasStateColumn){
        text[table] += spaces + spaces + spaces + "setStateByKey: mixins.instanceMethods.setStateByKey\n\n";
      }
      text[table] += spaces + spaces  + "},\n\n";
